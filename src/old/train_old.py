...
def create_model_2(number_of_inputs, number_of_classes, pretrained=True, device='cpu'):
    net = torchvision.models.segmentation.deeplabv3_resnet50(pretrained=pretrained)
    net.backbone['conv1'] = torch.nn.Conv2d(number_of_inputs, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3),
                                            bias=False)
    net.classifier[-1] = torch.nn.Conv2d(256, number_of_classes, kernel_size=(1, 1), stride=(1, 1))
    net = net.to(device)
    return net
...
def train_model(p: ParametersImage) -> None:
    ...
    model = create_model('deeplabv3_resnet50', n_inputs, n_outputs, pretrained=False)
